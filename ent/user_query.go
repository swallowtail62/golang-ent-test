...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withTeams; query != nil {
		...
		for _, n := range neighbors {
			fk := n.user_teams
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_teams" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_teams" returned %v for node %v`, *fk, n.ID)
			}
